...
设备已有的进程例外、DNS、MITM、节点或策略组；Apple Private Relay 住宅绑定已撤销，
但官方订阅按最新澄清恢复并保留，使用 `United States`，不得再次被模板删去。
...
Private Relay 住宅例外，不再把 Apple 与 PayPal 的出口一致性作为当前要求。用户随后明确
“保留 Sukka 全部”，故误删的 Private Relay 官方订阅已恢复；这是保留原先启用的公共资源，
不是恢复住宅覆盖或额外启用其他可选专项。
