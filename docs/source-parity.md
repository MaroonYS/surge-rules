...
| Apple Software Updates 补链 | +16 | 5 个 watchOS 核心主机继续内联；其余官方精确主机进入独立 `DOMAIN-SET` |
| 当前 DOMAIN-SET 条目 | 897 | 27 个本仓库 `DOMAIN-SET` |
| 当前 RULE-SET 条目 | 21 | 3 个无策略列 `RULE-SET` |
| 当前活动条目 | 918 | 30 个本仓库活动文件 |

[surge-expanded.conf](../surge-expanded.conf) 将当前自维护活动规则全部内联。
当前远程版共加载 30 个本仓库文件：27 个域名 `DOMAIN-SET`，以及承载
...
- `surge-main.conf`：通过 30 个远程文件加载当前活动规则。
