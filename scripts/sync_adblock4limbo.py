...
def parse_adblock(text: str) -> tuple[set[DomainRule], dict[str, int]]:
    ...
    for line_number, raw_line in enumerate(text.splitlines(), 1):
        ...
        if (
            domain not in EXCLUDED_DOMAIN_NAMES
            and validate.validate_domain(serialized) is not None
        ):
...
def parse_ruleset_domains(text: str) -> tuple[set[str], set[str]]:
    ...
    for line_number, raw_line in enumerate(text.splitlines(), 1):
        ...
        if (
            ...
            raise ValueError(
                f"invalid baseline RULE-SET domain at line {line_number}: "
                f"{raw_line}"
            )
        if rule_type == "DOMAIN-SUFFIX":
            ...
            exact.add(domain)
    if not exact and not suffix:
        raise ValueError("baseline RULE-SET has no usable DOMAIN records")
    return exact, suffix
...
def build(
    source_text: str,
    baseline_text: str,
    ruleset_baseline_text: str | None = None,
    ...
    if ruleset_baseline_text is None:
        ruleset_baseline_exact: set[str] = set()
        ruleset_baseline_suffix: set[str] = set()
    else:
        (
            ruleset_baseline_exact,
            ruleset_baseline_suffix,
        ) = parse_ruleset_domains(ruleset_baseline_text)

    remaining: set[DomainRule] = set()
    baseline_covered = 0
    ruleset_baseline_covered = 0
    explicitly_excluded = 0
    for entry in source_entries:
        if entry.domain in EXCLUDED_DOMAIN_NAMES:
            explicitly_excluded += 1
        elif covered_by_baseline(entry, baseline_exact, baseline_suffix):
            baseline_covered += 1
        elif covered_by_baseline(
            entry,
            ruleset_baseline_exact,
            ruleset_baseline_suffix,
        ):
            ruleset_baseline_covered += 1
        else:
            remaining.add(entry)
    ...
    stats = BuildStats(
        ...
        ruleset_baseline_covered=ruleset_baseline_covered,
    ...
    header = [
        ...
        f"# Additional baseline: {RULESET_BASELINE_URL}",
        ...
        (
            ...
            + ", ".join(f".{domain}" for domain in sorted(EXCLUDED_DOMAIN_NAMES))
        ),
        (
...
def source_metadata(
    source_text: str,
    baseline_text: str,
    ruleset_baseline_text: str,
) -> dict[str, dict[str, str]]:
    """Return volatile provenance without embedding it in the tracked rules."""
    return {
        "source": {
            "url": SOURCE_URL,
        ...
        "domain_set_baseline": {
            "url": BASELINE_URL,
            "sha256": hashlib.sha256(baseline_text.encode("utf-8")).hexdigest(),
        },
        "rule_set_baseline": {
            "url": RULESET_BASELINE_URL,
            "sha256": hashlib.sha256(
                ruleset_baseline_text.encode("utf-8")
            ).hexdigest(),
        },
    }


def effective_records(text: str) -> tuple[str, ...]:
    """Return the ordered rules that affect Surge matching semantics."""
    return tuple(
        line.strip()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--ruleset-baseline-file", type=Path)
    parser.add_argument("--output", type=Path, default=OUTPUT_PATH)
    parser.add_argument(
        "--metadata-json",
        type=Path,
        help="write downloaded source URLs and SHA-256 values outside the ruleset",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        ruleset_baseline_text = read_text(
            args.ruleset_baseline_file,
            RULESET_BASELINE_URL,
            args.timeout,
        )
        rendered, stats = build(
            source_text,
            baseline_text,
            ruleset_baseline_text,
        )
    ...
    if args.metadata_json is not None:
        try:
            ...
            print(f"cannot write {args.metadata_json}: {exc}", file=sys.stderr)
            return 1
    ...
    if args.write:
        try:
            ...
            return 1
        if effective_records(current) == effective_records(rendered):
            action = "Unchanged"
        else:
            try:
    ...
    print(
        ...
        f"{stats.explicitly_excluded} explicitly excluded, "
        f"{stats.baseline_covered} domain-set-baseline-covered removed, "
        f"{stats.ruleset_baseline_covered} rule-set-baseline-covered removed, "
